...
public class CouponDBDAO implements CouponDAO {
		...
		try {
			...
			for (Coupon c : getAllCoupons()) {
				if (c.getTitle().toLowerCase().equals(c1.getTitle().toLowerCase())) {
					System.out.println(new DuplicateException(c1, c1.getTitle()).getMessage());
				}
			}
	...
	public void updateCoupon(Coupon c1) {
		...
		try {
			...
			String sql = "UPDATE coupons " + "SET " + "TITLE=?," + "END_DATE=?," + "AMOUNT=?," + "TYPE=?,"
					+ "MESSAGE=?," + "PRICE=?," + "IMAGE=? " + "WHERE ID_=?;";
			myStmt = myCon.prepareStatement(sql);
